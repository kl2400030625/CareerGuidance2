...
import { getAdminResources } from '../lib/storage';
const RESOURCES = [
    {
...
export default function Resources() {
    ...
    return (<div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 pt-20 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {adminResources.length > 0 && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(featured.length > 0 ? featured : adminResources).slice(0, 6).map((r) => (<a key={r.id} href={r.link} className="card p-6 block hover:shadow-xl transition-all" target={r.link.startsWith('http') ? '_blank' : undefined} rel={r.link.startsWith('http') ? 'noreferrer' : undefined}>
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <h3 className="text-lg font-bold text-slate-900">{r.title}</h3>
                    {r.featured && (<span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">
                        Featured
                      </span>)}
                  </div>
        ...
        <motion.div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8" variants={containerVariants} initial="hidden" animate="visible">
          {RESOURCES.map((category, catIdx) => {
            ...
            return (<motion.div key={catIdx} variants={itemVariants} className="space-y-4">
                {/* Category Header */}
                ...
                <div className="space-y-3">
                  {category.items.map((item, idx) => (<motion.a key={idx} href={item.link} whileHover={{ x: 5 }} className="card p-5 block hover:shadow-xl transition-all cursor-pointer group">
        ...
        ].map((tip, idx) => (<motion.div key={idx} className="card p-6" whileHover={{ y: -5 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}>
        ...
        {/* CTA */}
        <motion.div className="mt-20 bg-gradient-to-r from-indigo-600 to-pink-600 rounded-2xl p-12 text-center text-white" whileHover={{ scale: 1.02 }} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
