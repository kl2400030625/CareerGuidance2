...
const COUNSELORS: Counselor[] = [
  {
    ...
    experienceYears: 10,
    contactEmail: 'ananya.rao@careerguide.edu',
  ...
  },
  {
    ...
    experienceYears: 7,
    contactEmail: 'rahul.mehta@careerguide.edu',
  ...
  },
  {
    ...
    experienceYears: 8,
    contactEmail: 'priya.sharma@careerguide.edu',
...
export default function Counseling() {
  ...
  const suggestedCareer = (location.state as { suggestedCareer?: string } | null)?.suggestedCareer

  const [selectedCounselorId, setSelectedCounselorId] = useState<number>(COUNSELORS[0]?.id ?? 1)
  ...
  useEffect(() => {
    if (!suggestedCareer) return
    setCareerInterest((current) => current || suggestedCareer)
  }, [suggestedCareer])

  const selectedCounselor = useMemo(
    () => COUNSELORS.find((c) => c.id === selectedCounselorId) ?? COUNSELORS[0],
    [selectedCounselorId],
  )
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          >
            ...
            {COUNSELORS.map((counselor) => (
              <button
                ...
                className={`w-full text-left card p-5 border-2 ${
                  selectedCounselorId === counselor.id ? 'border-indigo-500' : 'border-transparent'
                }`}
              >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="e.g. Software Engineer, Data Scientist, Higher Studies in USA"
                    className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-indigo-500"
                    required
                  />
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Share anything specific you want to discuss during the session..."
                    className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-indigo-500 resize-none"
                  />
                </div>
              </div>
              ...
              {successMessage && (
                <p className="mt-3 text-sm font-medium text-green-600 bg-green-50 border border-green-200 rounded-md px-3 py-2">
                  {successMessage}
                </p>
              )}
            </form>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {upcomingSessions.map((session) => (
                <div key={session.id} className="card p-4">
                  ...
                  {session.notes && (
                    <p className="text-xs text-gray-500 border-t border-gray-200 mt-2 pt-2 line-clamp-2">
                      {session.notes}
                    </p>
                  )}
                </div>
